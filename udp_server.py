...
from dnslib import A, DNSRecord, RCODE, RR
...
# TTL values for synthetic responses generated by this server.
BLOCK_TTL = 60
LOCAL_DB_TTL = 300
...
def handle_client_packet(server_sock, packet, client_addr, local_hosts):
    """Handle one client packet with blocklist, cache, local DB, and logging."""
    ...
    if domain in blocked_domains:
        ...
        if cached_response:
            ...
            if domain in local_hosts:
                ...
                # If no local mapping exists, return SERVFAIL in this feature-focused server.
...
def run_udp_dns_server(host=HOST, port=PORT):
    """Run a concurrent UDP DNS server with cache, blocklist, logging, and file DB."""
    local_hosts = load_local_hosts(LOCAL_HOSTS_PATH)

    server_sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    server_sock.bind((host, port))

    print(f"UDP DNS feature server listening on {host}:{port}")
